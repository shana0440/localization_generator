...
void main() {
  ...
  test('Test generate select message', () {
    final msg =
        generate("price", "100 {currency, select, TWD{NT} HKD{HK} other{\$}}");
    expect(msg,
  ...
  test('Test generate gender message', () {
    final msg = generate(
        "gender", "{gender, gender, female{female} male{male} other{other}}");
    expect(msg,
        """String gender({dynamic gender}) => Intl.gender(gender, female: Intl.message("female"), male: Intl.message("male"), other: Intl.message("other"))""");
  });

  test('Test generate plural message', () {
    final msg = generate(
        "plural", "{count, plural, =0{no reply} =1{1 reply} other{# replies}}");
    expect(msg,
        """String plural({dynamic count}) => Intl.plural(count, zero: Intl.message("no reply"), one: Intl.message("1 reply"), other: Intl.message("\$count replies"))""");
  });

  test('Test nested message', () {
    final msg = generate("party", """{gender_of_host, gender,
